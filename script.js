...
function evaluateHelper(v1,v2,op){
    if(op=='+')return v1+v2;
    if(op=='-')return v1-v2;
    if(op=='*')return v1*v2;
    if(op=='/')return v1/v2;
...
function evaluate(cellObject) {
    ...
    // infix evaluation
    // for(var i=0;i<formulaComponent.length;i++){
    //     console.log(typeof(formulaComponent[i]))
    // }
    var operator=[];
    var operand=[];
    let rVal=0;
        ...
        formula[i]=='/' || formula[i]=='-' ){
            if(formula[i]=='(') operator.push(formula[i]);
                ...
                while(operator[operator.length-1]!='('){
                    var op2=operand[operand.length-1];operand.pop();
                    var op1=operand[operand.length-1];operand.pop();
                    var op=operator[operator.length-1];operator.pop();
                    operand.push(evaluateHelper(op1,op2,op));
                }
                operator.pop();
            }else if(formula[i]=='+' || formula[i]=='-'){

            }
            // console.log(formula[i]+"op");
            }else{
            // console.log(formula[i]+ "2");
        }
    }
